...
const activeIds=new Set(active.map(c=>String(c.id)));const activeItems=D.itens.filter(i=>activeIds.has(String(i.contrato_id))&&i.status==='ATIVO');const cards=[['CONTRATOS COM VIGÊNCIA',active.length,'Contratos principais no período'],['CONTRATOS-ROTA ATIVOS',new Set(activeItems.map(i=>String(i.id))).size,'Itens ativos hoje vinculados aos contratos do período'],['ROTAS RR SEM / COM CONCLUSÃO',regular.filter(e=>!(e.iniciou&&e.terminou)).length+' / '+regular.filter(e=>e.iniciou&&e.terminou).length,'Sem conclusão inclui ausência de registro'],['DIÁRIAS ELEGÍVEIS',ds.length,'Uma por contrato e data'],['VALOR APURADO'+(pending?' · PARCIAL':''),money(dv+mv),pending+' parcelas aguardando conferência'],['PREVISÃO DO FECHAMENTO','A definir','Calendário futuro de atendimento não informado']];
...
const counts=types.map(([code,label,color])=>({label,color,n:fleetRows.filter(v=>v.tipo_contrato_locacao===code).length}));counts.push({label:'Outros / não informado',color:'#8292a0',n:fleetRows.filter(v=>!types.some(t=>t[0]===v.tipo_contrato_locacao)).length});
let angle=0;const segments=counts.filter(x=>x.n).map(x=>{const begin=angle;angle+=x.n/Math.max(fleetRows.length,1)*100;return x.color+' '+begin+'% '+angle+'%';});
$('ct-fleet-pie').innerHTML=fleetRows.length?'<div role="img" aria-label="Distribuição dos vínculos da frota" style="width:170px;height:170px;border-radius:50%;margin:auto;background:conic-gradient('+segments.join(',')+')"></div>'+counts.map(x=>'<p><span style="color:'+x.color+'">●</span> '+esc(x.label)+': <b>'+x.n+'</b> ('+(x.n/fleetRows.length*100).toFixed(1)+'%)</p>').join(''):'<p>Sem veículos no cadastro.</p>';
