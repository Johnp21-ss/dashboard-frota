...
#ct .ct-bottom.ct-pair{grid-template-columns:repeat(2,minmax(0,1fr));align-items:stretch}
#ct .ct-kpi:nth-child(6),#ct .ct-kpi:nth-child(7){grid-column:span 3}
#ct .ct-kpis{align-items:stretch}
#ct .ct-daily-scroll{overflow-x:auto;scrollbar-color:#7796ae #183043;max-width:100%}
#ct .ct-daily-scroll svg{display:block;font-family:system-ui,sans-serif}
#ct #ct-fleet-card h2{font-size:13px;letter-spacing:.025em;margin:0 0 10px}
#ct #ct-fleet-card>span{margin-top:8px}
#ct #ct-fleet-pie{display:grid;grid-template-columns:80px minmax(0,1fr);column-gap:10px;align-items:center}
#ct #ct-fleet-pie>div[role="img"]{width:80px!important;height:80px!important;grid-row:1 / span 5}
#ct #ct-fleet-pie p{font-size:14px;line-height:1.4;margin:2px 0}
#ct #ct-fleet-pie p b{font-size:14px}
#ct .ct-kpi strong{font-size:28px}
@media(max-width:1150px){#ct .ct-kpi:nth-child(6),#ct .ct-kpi:nth-child(7){grid-column:span 1}#ct .ct-bottom.ct-pair{grid-template-columns:1fr}}
#ct .ct-bottom.ct-wide{grid-template-columns:minmax(0,1fr)}#ct .ct-supplier-scroll{max-height:360px;overflow-y:auto;overflow-x:hidden;padding-right:12px;scrollbar-color:#7796ae #183043}#ct #ct-suppliers .ct-bar label span:first-child{white-space:nowrap;overflow:hidden;text-overflow:ellipsis}
</style>
...
<div class="ct-bottom ct-wide"><article class="ct-card"><h2>Diárias confirmadas — variação dia a dia</h2><div id="ct-daily" class="ct-daily-scroll"></div><p class="ct-note">Valor do dia acima do ponto; quantidade de contratos-rota diários com conclusão registrada acima da data. O valor é apurado uma vez por contrato principal e data. Arraste horizontalmente para ver todo o período.</p></article></div><div class="ct-bottom ct-pair"><article class="ct-card"><h2>Valor apurado por prestador</h2><div id="ct-suppliers" class="ct-supplier-scroll" tabindex="0" aria-label="Lista completa de prestadores"></div></article><article class="ct-card"><h2>Veículos com registro no período</h2><div id="ct-fleet"></div><p class="ct-note">Não representa disponibilidade ou estado de manutenção.</p></article></div>
...
const fleetCard=$('ct-fleet-card');
const C=new Map(D.vinculos.map(c=>[String(c.id),c])),I=new Map(D.itens.map(i=>[String(i.id),i])), vehicleContracts=new Map();
D.itens.forEach(i=>{if(i.veiculo_id!=null&&i.status==='ATIVO'){const k=String(i.veiculo_id);if(!vehicleContracts.has(k))vehicleContracts.set(k,new Set());vehicleContracts.get(k).add(String(i.contrato_id));}});
...
const extraContracts=e=>[...(vehicleContracts.get(String(e.veiculo_execucao_id))||[])].map(id=>C.get(id)).filter(c=>c&&c.status==='A'&&valid(c,day(e.data)));
...
const rows=D.escalas.filter(e=>day(e.data)>=a&&day(e.data)<=b),regular=rows.filter(e=>e.tipo_rota==='RR'),extras=rows.filter(e=>extra(e)&&e.iniciou&&extraContracts(e).length>0),ds=D.diarias.filter(d=>d.data>=a&&d.data<=b),active=D.contratos.filter(c=>overlap(c,a,b));
...
table('ct-extras',['Data','ID','Placa','Motorista','Tipo informado','Contratos ativos pelo veículo','Sinalização','Encerramento','KM registrado','Observação','Valor'],extras.map(e=>{const v=extraContracts(e);return[br(e.data),e.id,e.placa,e.motorista,names[e.tipo_rota],v.map(c=>'#'+c.id).join(', '),v.length>1?'Mais de um contrato ativo — conferir':'Associado pelo veículo',e.terminou?'Registrado':'Sem encerramento registrado',e.km_executado,e.observacao,'Aguardando lançamento'];}));
...
const dailyMap=new Map();ds.forEach(d=>{if(!dailyMap.has(d.data))dailyMap.set(d.data,{count:0,value:0,pending:0});const x=dailyMap.get(d.data);x.count++;if(d.valor===null)x.pending++;else x.value+=Number(d.valor);});
const confirmedItems=new Map();regular.filter(e=>e.iniciou&&e.terminou).forEach(e=>{const item=I.get(String(e.contrato_rota_id)),c=C.get(String(e.contrato_id)),d=day(e.data);if(!item||item.modalidade_pagamento!=='DIARIA'||!c||!valid(c,d)||!dailyMap.has(d))return;if(!confirmedItems.has(d))confirmedItems.set(d,new Set());confirmedItems.get(d).add(String(item.id));});
const dailySeries=[];for(let dt=new Date(a+'T12:00:00Z');dt.toISOString().slice(0,10)<=b;dt.setUTCDate(dt.getUTCDate()+1)){const d=dt.toISOString().slice(0,10),v=dailyMap.get(d)||{count:0,value:0,pending:0};dailySeries.push({date:d,...v,items:confirmedItems.get(d)?.size||0});}
const dailyMax=Math.max(...dailySeries.map(x=>x.value),1),step=150,chartWidth=Math.max(450,dailySeries.length*step),xpos=i=>75+i*step,ypos=v=>195-v/dailyMax*130;
const dailySvg='<svg width="'+chartWidth+'" height="295" viewBox="0 0 '+chartWidth+' 295" role="img" aria-label="Valores diários apurados, contratos confirmados e datas">'+
[0,.5,1].map(f=>'<line x1="15" x2="'+(chartWidth-15)+'" y1="'+ypos(f*dailyMax)+'" y2="'+ypos(f*dailyMax)+'" stroke="#385367" stroke-dasharray="4 4"/>').join('')+
'<polyline fill="none" stroke="#21b8eb" stroke-width="3" points="'+dailySeries.map((d,i)=>xpos(i)+','+ypos(d.value)).join(' ')+'"/>'+
dailySeries.map((d,i)=>'<g><title>'+esc(br(d.date)+': '+d.items+' contratos-rota; '+d.count+' contratos principais; '+money(d.value)+(d.pending?' (parcial; '+d.pending+' a conferir)':''))+'</title><text x="'+xpos(i)+'" y="'+(ypos(d.value)-24)+'" text-anchor="middle" fill="'+(d.pending?'#ffd78c':'#f4f8fc')+'" font-size="14" font-weight="600">'+esc(d.pending&&d.pending===d.count?'A conferir':money(d.value)+(d.pending?' *':''))+'</text><circle cx="'+xpos(i)+'" cy="'+ypos(d.value)+'" r="5" fill="'+(d.pending?'#ffc45e':'#21b8eb')+'"/><text x="'+xpos(i)+'" y="248" text-anchor="middle" fill="#f4f8fc" font-size="15" font-weight="600">'+d.items+' contratos-rota</text><text x="'+xpos(i)+'" y="276" text-anchor="middle" fill="#d0dce6" font-size="14">'+esc(br(d.date))+'</text></g>').join('')+'</svg>';
$('ct-daily').innerHTML=dailySvg+(dailySeries.some(d=>d.pending)?'<p class="ct-note">* Valor parcial: existem diárias com composição a conferir. Pontos representam apenas valores disponíveis, não valores pendentes.</p>':'');
const suppliers=new Map();const supplier=c=>{if(!suppliers.has(c.prestador))suppliers.set(c.prestador,{value:0,pending:0});return suppliers.get(c.prestador);};active.forEach(supplier);const add=(c,v)=>{const x=supplier(c);if(v===null)x.pending++;else x.value+=Number(v);};ds.forEach(d=>add(C.get(String(d.contrato_id)),d.valor));ms.forEach(m=>add(m.c,m.valor));const sorted=[...suppliers].sort((x,y)=>y[1].value-x[1].value||x[0].localeCompare(y[0],'pt-BR')),smax=Math.max(...sorted.map(x=>x[1].value),1);$('ct-suppliers').innerHTML=sorted.map(([n,x])=>'<div class="ct-bar"><label><span title="'+esc(n)+'">'+esc(n)+'</span><span>'+esc(x.pending&&x.value===0?'A conferir':money(x.value)+(x.pending?' *':''))+'</span></label><i style="width:'+Math.max(0,x.value/smax*100)+'%"></i>'+(x.pending?'<small>'+x.pending+' parcelas a conferir</small>':'')+'</div>').join('')||'<p>Sem prestadores no período.</p>';
