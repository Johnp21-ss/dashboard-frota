...
<div class="ct-contract-cards" id="ct-contract-cards"></div><div class="ct-kpis" id="ct-kpis"></div>
...
const activeIds=new Set(active.map(c=>String(c.id)));const activeItems=D.itens.filter(i=>activeIds.has(String(i.contrato_id))&&i.status==='ATIVO');const modalityCards=[['MENSAL','Contratos mensais','mês',mv],['DIARIA','Contratos diários','dia',dv]].map(([mode,title,unit,amount])=>{
const items=activeItems.filter(i=>i.modalidade_pagamento===mode),ids=new Set(items.map(i=>String(i.contrato_id))),gross=sum(items.map(i=>i.valor_unitario));
const incomplete=items.some(i=>i.valor_unitario==null),pendingCount=mode==='MENSAL'?ms.filter(x=>x.valor===null).length:ds.filter(x=>x.valor===null).length;
return '<article class="ct-card ct-kpi"><h2>'+title+'</h2><span>'+ids.size+' contratos · '+new Set(items.map(i=>String(i.id))).size+' contratos-rota ativos</span><strong>'+money(gross)+'</strong><span>Bruto cadastrado / '+unit+(incomplete?' · valores incompletos':'')+'</span><span class="ct-base">Estimativa base: '+money(gross)+' / '+unit+'<br>Apurado no período: '+money(amount)+(pendingCount?' · parcial ('+pendingCount+' parcelas a conferir)':'')+'</span></article>';
});$('ct-contract-cards').innerHTML=modalityCards.join('');
const cards=[['CONTRATOS COM VIGÊNCIA',active.length,'Contratos principais no período'],['CONTRATOS-ROTA ATIVOS',new Set(activeItems.map(i=>String(i.id))).size,'Itens ativos hoje vinculados aos contratos do período'],['ROTAS RR SEM / COM CONCLUSÃO',regular.filter(e=>!(e.iniciou&&e.terminou)).length+' / '+regular.filter(e=>e.iniciou&&e.terminou).length,'Sem conclusão inclui ausência de registro'],['DIÁRIAS ELEGÍVEIS',ds.length,'Uma por contrato e data'],['VALOR APURADO'+(pending?' · PARCIAL':''),money(dv+mv),pending+' parcelas aguardando conferência'],['PREVISÃO DO FECHAMENTO','A definir','Calendário futuro de atendimento não informado']];
